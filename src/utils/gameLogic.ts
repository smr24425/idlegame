import { Player, Equipment, Boss, PetConfig, PetEffectType, ArtifactConfig, Rarity } from '../types/game';
...
export const MEGA_PET_STAT_VALUES: Record<string, number> = {
  attack: 0.6,
  defense: 0.6,
  health: 0.6,
  critRate: 0.03,
...
export const getActivePetBonus = (player: Player, effectType: PetEffectType): number => {
  ...
  } else if (config.effectType === effectType) {
    return config.baseValue * (1 + pet.level);
  }
...
export const getTotalStats = (player: Player) => {
  ...
  if (player.megaPet && player.megaPet.unlocked) {
    player.megaPet.slots.forEach(slot => {
      if (slot.type) {
        // @ts-ignore
        megaPetStats[slot.type] += MEGA_PET_STAT_VALUES[slot.type] * player.megaPet.level;
      }
    });
  }
